    ...
    private void Update()
    {
        ...
        if (win==0)
        {
            ...
            if (manager.iteratingEnemy.eliteBattleTemp == manager.iteratingEnemy.attackType.Length)
            {
                ...
                manager.iteratingEnemy = null;
                FMODUnity.RuntimeManager.PlayOneShot("event:/Enemy/Gem_Broke_End");
                manager.SetState(PlayerState.IDLE);
            }
            else
            {
                //보석 깨지는 소리 투입할 곳
                Debug.Log("보석 깨짐");                                                  //진입 자체를 안함 여기
                manager.SetState(PlayerState.NEXTBATTLE);
                FMODUnity.RuntimeManager.PlayOneShot("event:/Enemy/Gem_Broke");          //진입 자체를 안함 여기
            }
        ...
        else if(win==2)
        {
            ...
            {
                manager.SetState(PlayerState.NEXTBATTLE);
                FMODUnity.RuntimeManager.PlayOneShot("event:/Enemy/Gem_Broke");
            }
